// ProCabinet — Quote sidebar editor (carved out of src/quotes.js). The full
// drill-in editor: renderQuoteEditor, field updates with debounce, autosave
// (_qMarkDirty / _qAutoSaveTimer), quote-number search suggest, client picker
// (+ "Add new client" popup), new-quote / exit / load flows
// (loadQuoteIntoSidebar, _qNewQuote, createQuoteFromEditor, saveQuoteEditor),
// and the print/doc dispatchers printQuote / printOrderDoc (they call the
// _build*PDF builders in src/cutlist-pdf.js as runtime globals).
//
// Top-level state declared here: _qFieldDebounceTimers, _Q_EMPTY_ICON,
// _qAutoSaveTimer — plus one document click listener (suggest-list blur) and
// the window._qNewQuoteFromInput export; none of that reads other files at
// parse time. List view, data, statuses and lifecycle CRUD (duplicateQuote,
// removeQuote, convertQuoteToOrder) stay in src/quotes.js.
//
// Cross-file dependencies (runtime, resolved through the global env):
//   - quotes / quoteClient / _quoteLabel / _lineSubtotal / _refreshQuoteTotals /
//     _nextQuoteNumber / _isDraftQuote / renderQuoteMain (src/quotes.js)
//   - _qpState / _lineRowHtml / _renderQuoteLineTotals / _lineUpdate
//     (src/line-editor.js)
//   - _printInFrame / _buildQuotePDF / _buildWorkOrderPDF / _buildOrderDocPDF
//     (src/cutlist-pdf.js)
//   - orders (src/orders.js); clients (src/clients.js); stockItems (src/stock.js)
//   - _db / _userId (src/db.js); _toast / _confirm / _byId / _popupVal /
//     _setSaveStatus / _openPopup / _closePopup (src/ui.js)
//   - _escHtml (src/cabinet.js); _posSuggest (src/cabinet-library.js — loads
//     later in tag order; runtime-only use)
//   - _enforceFreeLimit / _realCount (src/limits.js); getBizInfo (src/business.js)
//   - dimsLabelFromMM (src/units.js); _linePhotoUrls (src/line-photos.js)
//   - switchSection (src/settings.js); _requireAuth (src/app.js)
