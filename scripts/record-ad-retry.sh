...
node scripts/record-wiki-clips.mjs schedule-priority live-link-tour >> "$LOG" 2>&1 || { echo "RECORD FAILED" >> "$LOG"; exit 1; }
node scripts/postprocess-wiki-clips.mjs schedule-priority live-link-tour >> "$LOG" 2>&1 || { echo "ENCODE FAILED" >> "$LOG"; exit 1; }
echo "ALL DONE" >> "$LOG"
