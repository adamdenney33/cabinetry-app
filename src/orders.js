  ...
  const orderCard = o => {
    ...
    <div class="order-card${isOverdue ? ' order-overdue' : ''}${isEditing ? ' editing' : ''}" style="cursor:pointer" onclick="loadOrderIntoSidebar(${o.id})">
      ...
      <div class="oc-footer">
        ...
        ${typeof _toggleOrderThread === 'function' ? (() => { const _u = typeof _dealUnreadCount === 'function' ? _dealUnreadCount('order', o.id) : 0; const _cls = typeof _dealMsgBtnClass === 'function' ? _dealMsgBtnClass('order', o.id) : 'btn btn-outline'; return `<button class="${_cls}" data-msg-btn-order="${o.id}" onclick="event.stopPropagation();_toggleOrderThread(${o.id})" style="font-size:11px;padding:5px 8px;width:auto">Messages <span data-order-unread="${o.id}">${_u ? `(${_u})` : ''}</span></button>`; })() : ''}
