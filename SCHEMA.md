...
create table public.cutlists (
  ...
  cabinet_id  bigint references public.cabinet_templates(id) on delete cascade,         -- added 2026-05-12; library cut lists scoped to a saved cabinet
  name        text not null,
...
**`cabinet_id` (added 2026-05-12):** references `cabinet_templates(id)`, not `cabinets(id)` — the in-app "cabinet library" lives in `cabinet_templates` (see `_saveCabinetToDB` / `_loadCabinetTemplatesFromDB` in `src/cabinet.js`). A library cut list (`project_id = NULL`) can be linked to one saved cabinet template; the Cabinet Library card's Cut List pill reads counts from this column. Originally pointed at `cabinets(id)` which caused FK violations on every link attempt — fixed in migration `20260512081913_cutlists_cabinet_id_fk_to_templates.sql`.
