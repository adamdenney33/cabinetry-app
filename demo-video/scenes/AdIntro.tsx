...
export const AdIntro: React.FC<{ dur: number }> = ({ dur }) => {
  ...
  return (
    <AbsoluteFill style={{ ...PINSTRIPES, fontFamily: FONT, alignItems: 'flex-start', justifyContent: 'center', padding: '0 120px', opacity: out }}>
      <div style={{ transform: `scale(${punch})`, transformOrigin: 'left center', textAlign: 'left' }}>
        <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: 4.5, textTransform: 'uppercase', color: C.accent, opacity: eyebrow, marginBottom: 28 }}>
          The workshop OS for cabinet makers
        </div>
        <div style={{ ...DISPLAY, fontSize: 92, color: '#fff', letterSpacing: '-2.4px' }}>
          <Word at={8}>Quote custom</Word>{' '}<Word at={14}>cabinetry</Word>
        </div>
        <div style={{ ...DISPLAY, fontSize: 92, letterSpacing: '-2.4px', marginTop: 6 }}>
          <Word at={22} style={{ color: C.accent }}>in minutes</Word>{' '}
          <Word at={34} style={{ color: 'rgba(255,255,255,0.45)' }}>... not hours.</Word>
        </div>
        <div style={{ marginTop: 44, opacity: interpolate(brand, [0, 1], [0, 1]), transform: `translateX(${interpolate(brand, [0, 1], [-18, 0])}px)` }}>
